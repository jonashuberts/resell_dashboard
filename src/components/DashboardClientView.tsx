...
export function DashboardClientView({
  ...
  const timeOptions = [
    ...
    { label: t("filter.time.year"), value: "year" },
    { label: t("filter.time.last_year"), value: "lastYear" },
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return transactions.filter((tx) => {
      ...
      if (timeRange !== "all") {
        ...
        if (timeRange === "month" && (txYear !== currentYear || txMonth !== currentMonth)) {
          return false;
        }
        if (timeRange === "year" && txYear !== currentYear) {
          return false;
        }
        if (timeRange === "lastYear" && txYear !== currentYear - 1) {
          return false;
        }
      }
      ...
      // 2. Category filter
      if (category !== "all") {
        const itemData = getItemData(tx);
        const itemCat = itemData?.category || "Keine Kategorie";
        if (itemCat !== category) {
          return false;
        }
      }
  ...
  }, [transactions, timeRange, category]);

  // Fast KPI aggregations
  const stats = useMemo(() => {
    let totalRevenue = 0;
    let totalExpenses = 0;
    const soldItems = new Set<string>();
    const categoryStats: Record<string, { revenue: number; expenses: number; solds: number }> = {};

    filteredTransactions.forEach((tx) => {
      const amt = Number(tx.amount) || 0;
      const itemData = getItemData(tx);
      const catName = itemData?.category || "Keine Kategorie";

      if (!categoryStats[catName]) {
        categoryStats[catName] = { revenue: 0, expenses: 0, solds: 0 };
      }
      ...
      if (tx.type === "Verkauf") {
        ...
        if (tx.item_id) {
          soldItems.add(tx.item_id);
          categoryStats[catName].solds += 1;
        }
      } else {
        totalExpenses += amt;
        categoryStats[catName].expenses += amt;
      }
    });

    const netProfit = totalRevenue - totalExpenses;
    const roiPercentage = totalExpenses > 0 ? (netProfit / totalExpenses) * 100 : 0;
    const avgProfitPerItem = soldItems.size > 0 ? netProfit / soldItems.size : 0;
    ...
    const topCategories = Object.entries(categoryStats)
      .map(([name, data]) => {
        ...
        return { name, profit, margin, solds: data.solds };
      })
      .filter((c) => c.solds > 0 || c.profit !== 0)
  ...
  const inventoryStats = useMemo(() => {
    const filteredItemIds = new Set(
      filteredTransactions.map((tx) => tx.item_id).filter(Boolean) as string[]
    );

    let activeItems = items;
    if (category !== "all" || timeRange !== "all") {
      activeItems = items.filter((i) => {
        if (category !== "all" && (i.category || "Keine Kategorie") !== category) {
          return false;
        }
        if (timeRange !== "all" && !filteredItemIds.has(i.id)) {
          return false;
        }
        return true;
      });
    ...
    const inStockCount = activeItems.filter((i) => i.status === "Auf Lager").length;
    const soldCount = activeItems.filter((i) => i.status === "Verkauft").length;
    const toShipCount = activeItems.filter((i) => i.status === "Verkauft (Muss versendet werden)").length;

    // Stock value
    const inStockIds = new Set(
      activeItems.filter((i) => i.status === "Auf Lager" || i.status === "In Reparatur").map((i) => i.id)
    );

    const stockValue = stockTxs.reduce((sum, tx) => {
    ...
    // Days to sell
    const soldItemDetails = activeItems.filter((i) => i.status === "Verkauft");
    let totalDays = 0;
    let countedSold = 0;

    soldItemDetails.forEach((item) => {
      const sellTx = transactions.find((t) => t.item_id === item.id && t.type === "Verkauf");
      if (sellTx && item.created_at) {
        const createdDate = new Date(item.created_at);
        const soldDate = new Date(sellTx.date);
        const diffDays = Math.max(0, Math.round((soldDate.getTime() - createdDate.getTime()) / (1000 * 60 * 60 * 24)));
        totalDays += diffDays;
        countedSold += 1;
      }
    });

    const avgDaysToSell = countedSold > 0 ? Math.round(totalDays / countedSold) : 0;
  ...
  }, [items, transactions, stockTxs, filteredTransactions, category, timeRange]);
  ...
  return (
    <div className="p-6 sm:p-10 max-w-7xl mx-auto space-y-8">
      ...
      {/* Header bar with typography & Instant Segmented controls */}
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative inline-flex items-center">
            ...
            <select
              ...
              className="appearance-none bg-zinc-900/90 border border-white/[0.1] hover:border-white/[0.2] rounded-xl pl-8.5 pr-8 py-1.5 text-xs font-medium text-zinc-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 cursor-pointer backdrop-blur-md transition-all"
            >
          ...
          <div className="inline-flex items-center p-1 rounded-xl bg-zinc-900/90 border border-white/[0.1] backdrop-blur-md">
            {timeOptions.map((opt) => {
                ...
                >
                  {isActive && (
                    <motion.div
                      ...
                      className="absolute inset-0 rounded-lg bg-zinc-800 border border-white/[0.15] shadow-sm"
                    />
                  )}
      ...
      {/* 3-Column Balanced Section */}
      <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 items-stretch">
        {/* Card 1: Top Categories */}
        <div className="apple-card p-6 flex flex-col justify-between relative overflow-hidden h-full">
          <div className="apple-card-glow" />
          <div>
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2.5">
                <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-amber-500/15 border border-amber-500/25 text-amber-400">
            ...
            <div className="space-y-3 mb-5">
              {stats.topCategories.length > 0 ? (
                stats.topCategories.map((cat, idx) => (
                  <div
                    key={cat.name}
                    className="flex items-center justify-between p-2.5 rounded-xl bg-zinc-900/60 border border-white/[0.06] hover:bg-zinc-900/90 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <span className="flex h-6 w-6 items-center justify-center rounded-lg bg-zinc-800 text-[11px] font-bold text-zinc-300 font-mono">
                        {idx + 1}
                      </span>
          ...
          <div className="pt-4 border-t border-white/[0.06]">
            <div className="flex items-center justify-between p-3 rounded-xl bg-zinc-950/70 border border-white/[0.08]">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-blue-500/15 border border-blue-500/25 rounded-xl text-blue-400">
                  <Clock className="h-4 w-4" />
                </div>
                <div>
                  <div className="text-xs font-medium text-zinc-200">{t("dashboard.days.title")}</div>
                  <div className="text-[10px] text-zinc-400">{t("dashboard.days.desc")}</div>
                </div>
              </div>
        ...
        <div className="apple-card p-6 flex flex-col justify-between relative overflow-hidden h-full">
          ...
          <div>
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2.5">
                <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-emerald-500/15 border border-emerald-500/25 text-emerald-400">
            ...
            <div className="space-y-2.5">
              {recentTxs.length > 0 ? (
                recentTxs.map((tx) => (
                  <div
                    key={tx.id}
                    className="flex items-center justify-between p-2.5 rounded-xl bg-zinc-900/60 border border-white/[0.06] hover:bg-zinc-900/90 transition-colors group"
                  >
        ...
        <div className="apple-card p-6 flex flex-col justify-between relative overflow-hidden h-full">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="p-3.5 rounded-xl bg-zinc-950/70 border border-white/[0.08] text-center flex flex-col items-center justify-center">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center bg-zinc-900/60 px-3.5 py-2.5 rounded-xl border border-white/[0.06]">
                <span className="text-xs text-zinc-300 font-medium">{t("dashboard.logistics.total")}</span>
                <span className="text-xs font-bold text-white font-mono">{inventoryStats.itemsCount}</span>
              </div>
              <div className="flex justify-between items-center bg-zinc-900/60 px-3.5 py-2.5 rounded-xl border border-white/[0.06]">
                <span className="text-xs text-zinc-300 font-medium">{t("dashboard.logistics.sold")}</span>
                <span className="text-xs font-bold text-emerald-400 font-mono">{inventoryStats.soldCount}</span>
              </div>
              <div className="flex justify-between items-center bg-zinc-900/60 px-3.5 py-2.5 rounded-xl border border-white/[0.06]">
                <span className="text-xs text-zinc-300 font-medium">{t("dashboard.logistics.stock")}</span>
...
function AppleKpiCard({ 
  ...
  return (
    <div className="apple-card p-5 sm:p-6 relative overflow-hidden group cursor-default">
      <div className="apple-card-glow" />
      <div className={`absolute top-0 right-0 w-36 h-36 bg-gradient-to-br ${accentGlow} pointer-events-none blur-2xl`} />
